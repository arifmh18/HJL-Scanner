...
class DashboardListAdapter(val context: Context, val list: MutableList<DashboardModel>) : RecyclerView.Adapter<DashboardListAdapter.ListViewHolder>()  {
    ...
    inner class ListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var itemContent = binding.itemContent
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        ...
        holder.itemName.text = data.paket
        holder.itemContent.setOnClickListener {
            if (SystemClock.elapsedRealtime() - mLastClickTime < 1000){ return@setOnClickListener }
            mLastClickTime = SystemClock.elapsedRealtime()
            val intent = Intent(context, DetailDataDashboard::class.java)
            intent.putExtra("namepaket", data.paket)
            intent.putExtra("datepaket", data.date)
            intent.putExtra("idpaket", data.id)
            (context as Activity).startActivity(intent)
        }
        holder.itemDelete.setOnClickListener {
            val intent = Intent("deleteData")
